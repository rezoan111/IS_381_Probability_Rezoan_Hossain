...
Streak length 1 = Kobe makes 1 hit in a row, then the streak ends with a miss → H then M.

Streak length 0 = Kobe makes 0 hits, meaning he misses right away → just M.
...
The distribution is right-skewed (mostly small streaks, few long).

Typical streak length is 0 (happened the most often), and the longest streak is 4.

The plot shows most streaks are 0–1, and longer streaks are rare.
...
```{r q3-unfair-coin}
# set seed so this stays the same when you Knit
set.seed(12161994)
...
In this simulation, heads came up `r sum(sim_unfair_coin == "heads")` times.
...
# set seed so results stay the same when you Knit
set.seed(12161995)

# 45% chance of "H", 55% chance of "M", repeated for 133 shots
...
Change the `prob` argument so "H" has probability 0.45 and "M" has probability 0.55, and set `size = 133` so it matches Kobe’s number of shots.

(See the updated `sim_basket <- sample(...)` code chunk above.)
...
```{r sim-streak-summary}
sim_streak %>% count(length) %>% arrange(desc(n))
...
ggplot(sim_streak, aes(x = length)) +
  geom_bar() +
  labs(title = "Simulated shooter streak lengths",
       x = "Streak length (hits in a row)",
       y = "Count")
```

The distribution is also right-skewed (mostly 0 and 1).

Typical streak length is 0, and the longest streak in this run is `r max(sim_streak$length)`.
...
It’s random sampling, so the overall shape should look similar (lots of 0s and 1s), but the exact bar heights and the longest streak can change each time you run it.
...
Kobe’s streak lengths and the simulated shooter’s streak lengths look pretty similar: both have lots of 0–1 streaks and only a few longer streaks.

Since the simulated shooter has independent shots (no hot hand) but still shows streaks, Kobe’s streak pattern doesn’t look very different from what randomness can produce. So I don’t see strong evidence that the hot hand model fits Kobe’s shots here.
